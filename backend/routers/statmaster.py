...
6. Important: the `events` table contains games from multiple years. The user might say 'this season', 'current season', or 'this year', which means `season_year = {current_season}`.
7. CRITICAL: By default, stats MUST be filtered for 'Regular Season' games only (type_id = 2) unless the user explicitly asks for Spring Training or Postseason. You MUST JOIN `season_types` on `e.season_year = st.season_year AND e.date >= st.start_date AND e.date <= st.end_date` and filter WHERE `st.type_id = 2`.
...
12. PITCHERS VS BATTERS FOR GAMELOGS: Use your internal MLB knowledge. If the player is primarily a pitcher (e.g., Chris Sale, Paul Skenes), query `event_boxscores_pitching p` and include pitching stats (`p.ip`, `p.h`, `p.r`, `p.er`, `p.bb`, `p.k`, `p.hr`). If the player is a batter or a two-way player like Shohei Ohtani (unless pitching is explicitly requested), query `event_boxscores_batting b` and include batting stats (`b.ab`, `b.r`, `b.h`, `b.hr`, `b.rbi`, `b.bb`, `b.k`, `b.sb`).
13. CRITICAL FOR ALL QUERIES: You must ALWAYS add `LIMIT 25` to the end of every single query (e.g. gamelogs, team stats, historical records) unless the user explicitly asks for more. Returning hundreds of rows will crash the frontend.
