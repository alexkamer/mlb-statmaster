...
# TARGET DATE: April 4, 2026 (Yesterday)
...
print("2. Training XGBoost Model on data exclusively before April 4th...")
...
print("\n3. Backtesting against Yesterday's Slate (2026-04-04):\n")
if len(df_yesterday) == 0:
    print("No games found for yesterday.")
else:
    df_yesterday['predicted_k'] = model.predict(df_yesterday[final_features]).round(1)
    df_yesterday = df_yesterday.sort_values('date')
    
    print(f"{'Pitcher':<20} | {'Expected':<8} | {'Vegas O/U':<9} | {'Our Bet':<10} | {'Actual K':<8} | {'Result':<10}")
    print("-" * 75)
    
    wins, losses, pushes, no_bets = 0, 0, 0, 0
    
    for _, row in df_yesterday.iterrows():
        # Get actual Pitcher Strikeout O/U from the database
        ...
        real_ou = float(prop_res.iloc[0]['prop_line']) if not prop_res.empty else None
        
        if real_ou is None:
            continue # Skip games without Vegas odds
            
        # Determine the Bet (Requires a 0.3 K edge to place a bet, otherwise we pass)
        predicted_k = row['predicted_k']
        actual_k = row['target_k']
        
        if predicted_k >= (real_ou + 0.3): bet = "OVER"
        elif predicted_k <= (real_ou - 0.3): bet = "UNDER"
        else: bet = "PASS (Close)"
        
        # Did we win?
        if bet == "OVER" and actual_k > real_ou:
            result = "✅ WON"
            wins += 1
        elif bet == "UNDER" and actual_k < real_ou:
            result = "✅ WON"
            wins += 1
        elif actual_k == real_ou:
            result = "➖ PUSH"
            pushes += 1
        elif bet == "PASS (Close)":
            result = "N/A"
        ...
        print(f"{row['pitcher_name']:<20} | {predicted_k:<8.1f} | {real_ou:<9.1f} | {bet:<10} | {actual_k:<8.0f} | {result:<10}")
