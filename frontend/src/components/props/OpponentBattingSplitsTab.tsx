...
interface OpponentBattingSplitsTabProps {
    propType: string;
    propLine: string | number;
    opponentSplitLogs: any[];
    initialIsHome: boolean;
    splitOuts: number;
    setSplitOuts: (val: number) => void;
    initialOpponentId: string | null;
    initialOpponentAbbrev: string;
    fetchOpponentBattingSplits: (id: number, outs: number, year: number) => Promise<any>;
    setOpponentSplitLogs: (logs: any[]) => void;
}

export const OpponentBattingSplitsTab: React.FC<OpponentBattingSplitsTabProps> = ({ 
    propType, propLine, opponentSplitLogs, initialIsHome, splitOuts, setSplitOuts, 
    initialOpponentId, initialOpponentAbbrev, fetchOpponentBattingSplits, setOpponentSplitLogs 
}) => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6">
                <div className="flex items-center gap-4 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    />
                    <span className="font-black text-indigo-600 w-8">{splitOuts}</span>
                </div>

                {values.length === 0 ? (
                    <div className="text-center p-8 text-slate-500 font-bold">No split data available for this cutoff.</div>
                ) : (
                                    ...
                                    {values.map((v, i) => {
                                        ...
                                        return (
                                            <tr key={i} className="hover:bg-slate-50 transition-colors">
                                                <td className="p-4 font-bold text-slate-300 text-center">{i + 1}</td>
                                                <td className="p-4 font-bold text-slate-700 whitespace-nowrap">{new Date(v.game_date).toLocaleDateString(undefined, {month: 'short', day: 'numeric'})}</td>
                                                <td className="p-4 font-bold text-slate-500 flex items-center gap-2">
                                                    <div className="w-4 h-4 bg-slate-100 rounded-full border border-slate-200 overflow-hidden flex-shrink-0">
                                                        <SafeImage 
                                                            src={`https://a.espncdn.com/i/teamlogos/mlb/500/${v.opponent_id}.png`} 
                                                            alt="Opponent"
                                                            className="w-full h-full object-contain"
                                                            hideOnError
                                                        />
                                                    </div>
                                                    <span className={v.home_away === expectedOpponentLocation ? 'text-slate-800' : ''}>
                                                        {v.home_away === 'home' ? '@ ' : 'vs '}{v.opponent_abbrev || 'OPP'}
                                                    </span>
                                                </td>
